...
@section('content')
    ...
    <form action="{{ route('v1expenses.store') }}" method="POST" class="max-w-md mx-auto bg-white p-6 rounded-lg shadow-lg">
        ...
        <input type="number" id="price" name="items[0][price]" class="w-full p-2 border rounded mb-4" required>

        <label class="block font-semibold mb-2">Сумма</label>
        <input type="number" id="total" name="items[0][total]" class="w-full p-2 border rounded mb-4" readonly>
